...
/*
   ...
   This program will use message queues to notify the accessibility/availability of the files between the parent and the child processes.
...
struct message{
	long mtype; // PARENT expects 0, the CHILD expects its PID.
	int pid;
	int index;
};
...
int main()
{
	...
	struct message *msgChild, *msgParent;
	...
	int flag, child_counter = numberChild;
	...
	for(int i=0; i<numberChild; i++)
	{
		...
		if(pid == -1){
			...
			printf("Child process %d (PID: %d) will begin operations.\n",i+1, getpid());
			char filename[] = "";

			msgChild = malloc(sizeof(struct message)); 
			msgParent = malloc(sizeof(struct message));

			// WAIT UNTIL THERE ARE NO MORE UNHANDLED FILES.
			while(continueListening)
			{
				...
				for(int i = 0; i<file_count; i++)
				{
					// if file is perceived to be available, do a request to the parent to find out.
					if(file_status[i] == 0){
						// Child will have mtype=1.	
						msgChild->mtype=1;
						msgChild->pid=getpid();
						msgChild->index=i;
						//printf("\nThe child will send the following values %ld %d %d \n", msgChild->mtype, msgChild->pid, msgChild->index);						

						if(msgsnd(msgid, msgChild, sizeof(struct message) - sizeof(long), 0) == -1){
							perror("msgsnd() ha fallado en hijo.\n");
							printf("Valor Errno: %d\n", errno);
							printf("Descripcion de Errno: %s\n", strerror(errno));
							exit(1);
						}
				
						ssize_t received = msgrcv(msgid, msgParent, sizeof(struct message) - sizeof(long), getpid(), 0);
						if(received == -1){
							perror("msgrc() ha fallado en hijo\n");
							printf("Valor Errno: %d\n", errno);
							printf("Descripcion de Errno: %s\n", strerror(errno));
							exit(1);
						}
						...
						if(!msgParent->index){ // if 0 then true, means truly available
							flag=0;
							// GET DATA FROM TEXT:
							char *output = processText(file_list[i]);
							// WRITE INTO CSV FILE:
							writeCSV(output, file_lock, file_list[i]);
							file_status[i]=1;
							printf("Child (PID: %d) finished writing %s into output.csv\n", getpid(), file_list[i]);
					...
					if(i == file_count-1 && flag){
						continueListening=0;
						msgChild->mtype=2;
						msgChild->index=0;
						msgChild->pid=getpid();
						if(msgsnd(msgid, msgChild, sizeof(struct message) - sizeof(long),0) == -1){
							perror("ERROR EN MANDAR TERMINAR EJECUCION HIJO\n");
							exit(1);
						}
					}
					/*
					printf("children (PID: %d) ", getpid());
					for(int j=0; j<file_count; j++)
						printf("%d ", file_status[j]);
					printf("\n");
					*/
				}
